...
3. Enable Bluetooth Setup: In your Xcode swiftUI, click the folder button on your left sidebar, which is called "Show the Project Navigator". Select the "USBInterfaceApp", and a new screen on your main view will be popped out. Select the target as "USBInterfaceAPP" and select the "info" section. In the "Custom IOS Target Properties", in the key list, press "plus" and add the following key names: 
